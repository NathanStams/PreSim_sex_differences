...
int F_generic(const T** arg, T** res) {
	...
	for (int i = 0; i < 3; ++i) res[0][i + nCoordinates + 6] = value<T>(tibia_r_posInGround[i]);
	for (int i = 0; i < 3; ++i) res[0][i + nCoordinates + 9] = value<T>(tibia_l_posInGround[i]);
	for (int i = 0; i < 3; ++i) res[0][i + nCoordinates + 12] = value<T>(pedal_r_velInGround[i]);
	for (int i = 0; i < 3; ++i) res[0][i + nCoordinates + 15] = value<T>(pedal_l_velInGround[i]);
